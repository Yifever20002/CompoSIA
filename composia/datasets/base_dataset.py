...
class BaseDataset(Dataset):
    ...
    def __init__(
        ...
        i2v_random_mask_probs: Dict[Literal["first_image", "random_middle_image", "random_first_n_images", "drop_last"], float] = {"first_image": 1.0},  # random mask type for inpainting
    ...
    def get_mask(self, pixel_values_shape, mask_type, num_condition_images=None):
        ...
        # TODO(magic number): num_frames_per_latent
        s = 1
        if num_condition_images is not None:
            # fixed mode
            assert num_condition_images % s == 1, f"num_condition_images % {s} != 1"
